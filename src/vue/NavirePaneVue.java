 ...
 * @author ueve
 */
public class NavirePaneVue extends FlowPane {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            navireslbl[i].setOnMouseClicked(new EventHandler<MouseEvent>() {
                ...
                public void handle(MouseEvent event) {
                    //desactiverNav(x);
                    InitialisationVue.setNavEnCours(x);
                    rotateImage(x);
                    setSelected(true);
                }
            });
    ...
    public void rotateImage(int i) {
        if (Accueil.getPartie().getJoueur().getNavires().getPosition().equals("H")) {
            rt = new RotateTransition(Duration.millis(500), navireslbl[i]);
            rt.setByAngle(-90);
            rt.setAutoReverse(true);
            rt.play();
            Accueil.getPartie().getJoueur().getNavires().setPosition("V");
        } else {
            rt = new RotateTransition(Duration.millis(500), navireslbl[i]);
            rt.setByAngle(90);
            rt.setAutoReverse(true);
            rt.play();
            Accueil.getPartie().getJoueur().getNavires().setPosition("H");
        }
    }
    ...
    public  Label getLblEnCours(){
        return navireslbl[InitialisationVue.getNavEnCours()];
    }
